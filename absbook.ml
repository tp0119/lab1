...
(* reduce f list -- Applies f to the elements of list left-to-right
   ...
   an oversight in not appearing in the Stdlib or List modules. *)
...
(* repeat count f x -- Applies f x repeatedly, count times, ignoring
   all but the last application, and returning the result of the last
   application. *)
...
(* call_timed ?(count = 1) f x -- Applies f to x count times returning
   a pair of the last result and the time in milliseconds to execute
   the function call. *)
...
(* call_reporting_time f x -- Applies f to x count times returning the
   last result, reporting timing information on stdout as a side
   effect. *)
